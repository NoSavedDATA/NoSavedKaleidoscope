...
#define MMA_WARP_SIZE 32
#define WARPS_PER_BLOCK 8      // BLOCK_ROW_WARPS * BLOCK_COL_WARPS
#define MMA_THREADS_PER_BLOCK 256  // MMA_WARP_SIZE * WARPS_PER_BLOCK
...
#define CHUNK_COPY_LINES_PER_WARP 8  // MMA_WARP_SIZE * THREAD_COPY_BYTES / CHUNK_LINE_BYTES
#define CHUNK_COPY_LINE_LANES 4      // MMA_WARP_SIZE / CHUNK_COPY_LINES_PER_WARP
...
/////////////////////////////////////////
    ...
    extern __shared__ half hsmem[][AB_SMEM_STRIDE];

    const size_t warp_id = threadIdx.x / MMA_WARP_SIZE;
    const size_t lane_id = threadIdx.x % MMA_WARP_SIZE;

    constexpr size_t B_smem_idx_off = BLOCK_ROWS;
    constexpr size_t smem_stage_off = BLOCK_ROWS + BLOCK_COLS;

    half *smem_warp_tile_row_ptr = &hsmem[0][0] + (warp_id / BLOCK_ROW_WARPS) * C_SMEM_STRIDE * WARP_ROWS;
    const half *smem_warp_stream_ptr = &hsmem[0][0] + warp_id * MMA_M * 2 * C_SMEM_STRIDE;
...
#pragma unroll
    for (size_t i = 0; i < A_smem_iters; ++i) {
        uint32_t A_smem_lane_addr = __cvta_generic_to_shared(&hsmem[A_smem_idx][0]) +
...
#pragma unroll
    for (size_t i = 0; i < B_smem_iters; ++i) {
        uint32_t B_smem_lane_addr = __cvta_generic_to_shared(&hsmem[B_smem_idx][0]) +
                                    ((lane_id % CHUNK_COPY_LINE_LANES +
...
#pragma unroll
    for (size_t i = 0; i < WARP_COL_TILES; ++i) {
        ...
        uint32_t A_smem_lane_addr = __cvta_generic_to_shared(
            &hsmem[A_smem_idx + lane_id % 16][((lane_id / 16) * 8 + (lane_id % 16 % (PERMUTED_COLS * SMEM_BANK_ROWS)) /
...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        ...
        uint32_t B_smem_lane_addr =
            __cvta_generic_to_shared(&hsmem[B_smem_idx + lane_id % 8]
...
#pragma unroll
        for (size_t i = 0; i < WARP_COL_TILES; ++i) {
            ...
            uint32_t A_smem_lane_addr =
                __cvta_generic_to_shared(&hsmem[A_smem_idx + lane_id % 16]
...
void mmaAsyncStage3(half *x, half *W, half *o, size_t M, size_t N, size_t K) {
    // [M, K]
    // [K, N]
    // [M, N]
    ...
    dim3 block(MMA_THREADS_PER_BLOCK);

    mmaAsyncStage3Kernel<<<grid, block, smem_max_size>>>(x, W, o, M, N, K);
}
