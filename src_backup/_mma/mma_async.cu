...
// CHUNK_K 2  // 32 / MMA_K
// CHUNK_LINE_BYTES 64          // 32 * sizeof(half)
// CHUNK_COPY_LINES_PER_WARP 8  // warpSize * THREAD_COPY_BYTES / CHUNK_LINE_BYTES
// CHUNK_COPY_LINE_LANES 4      // warpSize / CHUNK_COPY_LINES_PER_WARP



CHUNK_LINE_BYTES / THREAD_COPY_BYTES



    const int A_smem_iters = 256 / (CHUNK_COPY_LINES_PER_WARP * WARPS);
    const int B_smem_iters = 128 / (CHUNK_COPY_LINES_PER_WARP * WARPS);
...
// A
        size_t A_smem_idx = by_warp_offset;
        int4 *A_lane_ptr = (int4 *)(A_warp_ptr + tile_k * MMA_K + mw*K) + ml;
...
#pragma unroll
        for (size_t i = 0; i < A_smem_iters; ++i) {
            ...
            // smem[A_smem_idx][0] + (( (A_smem_idx % (4 * SMEM_BANK_ROWS))/SMEM_BANK_ROWS + ml) % 4) * THREAD_COPY_BYTES; // for half

                
            smem[A_smem_idx][0] + (( A_smem_idx % 4 + ml) % 4) * THREAD_COPY_BYTES; // for float
            ...
            A_lane_ptr = (int4 *)((half *)A_lane_ptr + 8 * K);
            A_smem_idx += 8;
        }
...
// #pragma unroll
//             for (size_t i = 0; i < WARP_COL_TILES|4; ++i) {

//                 size_t A_smem_idx = (warp_id / BLOCK_ROW_WARPS) * WARP_ROWS|64 + i * MMA_M|16;
...
//             }
        ...
        for (size_t i = 0; i < wy_per_mma_n; ++i) {

            size_t A_smem_idx = warp_y*wy + i*MMA_PTX_M;


            uint32_t A_smem_lane_addr = __cvta_generic_to_shared(
                &smem[A_smem_idx + lane_id % 16]
                        [(k_step * MMA_K + (lane_id / 16) * 8 +
                         (lane_id % 16 % PERMUTED_COLS) * PERMUTED_OFFSET) % AB_SMEM_STRIDE]);
...
// BLOCK_ROW_WARPS 4  // BLOCK_ROWS / WARP_ROWS
// BLOCK_COL_WARPS 2  // BLOCK_COLS / WARP_COLS

// BLOCK_COL_TILES 16  // BLOCK_ROWS / MMA_M=16
// BLOCK_ROW_TILES 16  // BLOCK_COLS / MMA_N=8

// WARP_COL_TILES 4  // WARP_ROWS / MMA_M=16
// WARP_ROW_TILES 8  // WARP_COLS / MMA_N=8
