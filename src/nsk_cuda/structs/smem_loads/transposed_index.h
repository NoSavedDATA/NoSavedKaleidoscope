...
__device__ void smem_cpasync_wmma_loader<warp_rows_per_m, warp_cols_per_n, T>::load_A_transposed_indexed(T *x_smem, const float *embedding_book, const float *idxs, int next_tile, int M, int N) {
    for(int block_tile=0; block_tile<wmma_idx.by_per_w/4; ++block_tile) 
    {
        ...
        for(int i=0;i<4;++i)
        {
            ...
            if (row<M && col<N)
            {
              ...
              smem_out[i] = embedding_book[idx*N + col];
            }
            else
                smem_out[i] = 0.0f;
        } 
...
template<int warp_rows_per_m, int warp_cols_per_n, typename T>
__device__ void smem_cpasync_wmma_loader<warp_rows_per_m, warp_cols_per_n, T>::load_B_transposed_indexed(T *x_smem, const float *embedding_book, const float *idxs, int next_tile, int M, int N) {
