...
__device__ void blocking_tiled_wmma_fp16_16x16x16_dw(fp16_wmma_frags<warp_rows_per_m, warp_cols_per_n, __half> &frag_loader,
                                              ...
                                              const int M, const int N, const int K, const int WMMA_M, const int WMMA_N)
{
    ...
    smem_loader.load_B_transposed(w_smem, w, 0, K, N); // x  [B, C]
    ...
    for (int tile=0; tile<K; tile+=wmma_idx.wk)
    {
        ...
        if (next_tile<K)
        {
            ...
            smem_loader.load_B_transposed(w_smem, w, next_tile, K, N);
