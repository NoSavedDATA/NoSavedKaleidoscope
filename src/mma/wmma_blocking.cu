...
#define WMMA_K 16
  ...
  template<int WMMA_T, int wx_per_mma_m, int wy_per_mma_n, int wk>
  ...
  wmma::fragment<wmma::matrix_a, 16, 16, 16, __half, wmma::row_major> x_frag[wx_per_mma_m]; // wy_per_mma_n
  wmma::fragment<wmma::matrix_b, 16, 16, 16, __half, wmma::col_major> w_frag[wy_per_mma_n]; // wx_per_mma_m
  



  float   acc_frag[wx_per_mma_m*wy_per_mma_n*8];
  

  for (int i=0; i<wx_per_mma_m*wy_per_mma_n*8; ++i)
  {
    acc_frag[i] = 0.0f;
  }
...
/////////////////////////////
      ...
      for (int wx_tile=0; wx_tile<wx_per_mma_m; ++wx_tile)
      {
        ...
        for (wy_tile=0; wy_tile<wy_per_mma_n; wy_tile+=1)
        {
          if ((block_y*by + wy_tile*WMMA_N)<B && (block_x*bx + wx_tile*WMMA_M)<OC)
            wmma16x16x16(acc_frag+(wx_tile*wy_per_mma_n + wy_tile)*8, x_frag[wy_tile], w_frag[wx_tile]); // 8 is the frag ld
...
template<int WMMA_T>
inline void blocking_mma(const float *x, const float *w, float *o, int B, int C, int OC, cudaStream_t stream)
{
  ...
  switch (grid.wx_per_mma_m)
  {
    case 1:
      switch (grid.wy_per_mma_n)
      {
        case 1:
          wmma_blocking<WMMA_T, 1, 1, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 2:
          wmma_blocking<WMMA_T, 1, 2, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 3:
          wmma_blocking<WMMA_T, 1, 3, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 4:
          wmma_blocking<WMMA_T, 1, 4, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
    ...
    case 2:
      ...
      {
        case 1:
          wmma_blocking<WMMA_T, 2, 1, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 2:
          wmma_blocking<WMMA_T, 2, 2, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 3:
          wmma_blocking<WMMA_T, 2, 3, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 4:
          wmma_blocking<WMMA_T, 2, 4, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
    ...
    case 3:
      ...
      {
        case 1:
          wmma_blocking<WMMA_T, 3, 1, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 2:
          wmma_blocking<WMMA_T, 3, 2, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 3:
          wmma_blocking<WMMA_T, 3, 3, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 4:
          wmma_blocking<WMMA_T, 3, 4, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
    ...
    case 4:
      ...
      {
        case 1:
          wmma_blocking<WMMA_T, 4, 1, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 2:
          wmma_blocking<WMMA_T, 4, 2, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 3:
          wmma_blocking<WMMA_T, 4, 3, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
        case 4:
          wmma_blocking<WMMA_T, 4, 4, 32><<<grid.g, grid.w, grid.smem, stream>>>
                                  (x, w, o, B, C, OC, grid.b.x, grid.b.y, grid.wx, grid.wy,
                                  grid.bx_per_w, grid.by_per_w,
                                  grid.bx_per_wx, grid.by_per_wy);
          break;
