...
__global__ void wmma_blocking_dw(const float *__restrict__ x, const float *__restrict__ w,
                        ...
                        const int bx_per_wx)
 {
  ...
  blocking_tiled_wmma_fp16_16x16x16_dw(frag_loader, wmma_idx, smem_loader,
                                    x, w, x_smem, w_smem,
                                    M, N, K, WMMA_M, WMMA_N);
