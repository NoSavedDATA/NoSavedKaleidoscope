...
template<int WMMA_T>
void blocking_mma(const float *x, const float *w, float *o, int B, int C, int OC, cudaStream_t stream)
{
  ...
  using LaunchFn = void(*)(Grid2, const float*, const float*, float*, int, int, int, cudaStream_t);
  ...
  launcher(grid, x, w, o, B, C, OC, stream);
}
